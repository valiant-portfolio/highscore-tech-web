...
/**
 * An analyst's reading of one order: a sentence, and optionally her marked-up
 * chart.
 *
 * Written while the order is still PENDING — that is the point of the routine.
 * "We only analyze pending trades; when a trade is triggered it means we
 * haven't confirmed it" (Victor, 25 Sep 2026).
 *
 * Keyed by broker ticket, which in MT5 is the same number for the pending
 * order and the position it becomes. So the note written before the trigger is
 * still attached after the fill, after the close, and on the post-mortem page
 * months later.
 *
 * `context` records what the order WAS when it was read, so the note can be
 * judged against the setup it describes rather than against whatever the
...
export async function saveTradeAnalysisAction(
  ...
  if (image) {
    ...
    // 10MB is far above any screenshot and far below anything that would wedge
    // the upload — a phone photo of a screen is ~3MB.
    ...
    // Ticket-prefixed so everything about one order sits together in the
